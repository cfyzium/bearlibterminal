...
void TestOutput()
{
	...
	terminal_print(2, 5, L"[color=orange]3.[/color] Unicode examples: Кириллица Ελληνικά α=β²±2°");
...
struct UnicodeRange
{
	...
	static std::vector<UnicodeRange> g_wgl4_ranges =
	{
...
std::vector<UnicodeRange> GetWGL4Ranges()
{
	auto ranges = g_wgl4_ranges;
	return ranges;
}
...
void TestFontViewer()
{
	...
	auto param_selection = [&](int i) -> const char*
	{
		return current_param == i? "[U+203A]": " ";
	};
	...
	for (bool proceed=true; proceed;)
	{
		...
		more = current_range < g_wgl4_ranges.size()-1;
		terminal_printf
		(
			1, 4,
		...
		do
		{
			...
			else if (key == TK_UP && current_param > 0)
			{
				current_param -= 1;
			}
			else if (key == TK_DOWN && current_param < 3)
			{
				current_param += 1;
			}
			else if (key == TK_LEFT || key == TK_RIGHT)
			{
				...
				{
					// Character range
					if (key == TK_LEFT && current_range > 0)
					{
						current_range -= 1;
					}
					else if (key == TK_RIGHT && current_range < g_wgl4_ranges.size()-1)
					{
						current_range += 1;
					}
				}
...
void TestManualCellsize()
{
	...
	auto setup_scene = [&]()
	{
		terminal_setf
		(
			"font: UbuntuMono-R.ttf, size=%d; window.cellsize=%dx%d",
